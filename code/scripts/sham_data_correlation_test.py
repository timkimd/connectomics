...
pred_corr, pred_p = stats.pearsonr(y, y_pred)
print(f"\nPrediction Performance:")
print(f"Correlation between actual and predicted: {pred_corr:.4f}")
...
axes[0].set_xlabel('Actual Firing Rates')
axes[0].set_ylabel('Predicted Firing Rates')
axes[0].set_title(f'Actual vs Predicted\n(r = {pred_corr:.3f})')

# Plot 2: Residuals vs Predicted
axes[1].scatter(y_pred, residuals, alpha=0.6)
axes[1].axhline(y=0, color='r', linestyle='--')
axes[1].set_xlabel('Predicted Firing Rates')
...
def statsmodels_cv(X, y, cv_folds=5, random_state=42):
    """
    Perform cross-validation using statsmodels OLS
    """
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        # Calculate R-squared manually for test set
        ss_res = np.sum((y_test - y_pred) ** 2)
        ss_tot = np.sum((y_test - np.mean(y_test)) ** 2)
        r_squared = 1 - (ss_res / ss_tot)

        # Store results
        cv_results['r_squared'].append(r_squared)
        cv_results['adj_r_squared'].append(model.rsquared_adj)  # From training
        ...
        print(f"Fold {fold + 1}: R² = {r_squared:.4f}, RMSE = {rmse:.4f}")

    return cv_results


# Perform cross-validation
print("Statsmodels OLS Cross-Validation:")
print("=" * 40)
cv_results = statsmodels_cv(X, y, cv_folds=5)

# Calculate summary statistics
cv_r2_mean = np.mean(cv_results['r_squared'])
cv_r2_std = np.std(cv_results['r_squared'])
cv_rmse_mean = np.mean(cv_results['rmse'])
cv_rmse_std = np.std(cv_results['rmse'])

print(f"\nCross-Validation Summary:")
print(f"Mean R²: {cv_r2_mean:.4f} ± {cv_r2_std:.4f}")
print(f"Mean RMSE: {cv_rmse_mean:.4f} ± {cv_rmse_std:.4f}")
...
for i, name in enumerate(param_names):
    print(f"{name}: {mean_coeffs[i]:.4f} ± {std_coeffs[i]:.4f}")


#%% Advanced cross-validation with detailed statistics i.e. Roid results
def detailed_statsmodels_cv(X, y, cv_folds=5, random_state=42):
    """
...
detailed_results = detailed_statsmodels_cv(X, y, cv_folds=5)
...
coeff_df = pd.DataFrame([result['model_summary']['params'] for result in detailed_results])
...
comparison_data = {
    ...
    'OLS': [
        ...
        f'{rmse_ols:.4f}',
        f'{mae_ols:.4f}',
        f'{np.mean(ols_cv_scores):.4f}',
        f'{np.std(ols_cv_scores):.4f}',
        f'{ols_model.aic:.2f}',
...
print(
    f"\nCross-validation shows {'Poisson GLM' if cv_pseudo_r2_mean > np.mean(ols_cv_scores) else 'OLS'} performs better on average")
