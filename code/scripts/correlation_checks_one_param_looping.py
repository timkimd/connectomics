...
for col in X.columns:
    ...
    ols_cv_scores = OLS_CV(X[col], y, cv_folds=5)
...
for col in X.columns:
    status = "LOO"
    cv_results = fit_beta_model_with_cv(X[col], y, cv_folds=X.shape[0])
    ...
    for i in range(times_to_shuffle):
        ...
        X_shuffled = X[col].sample(frac=1, random_state=random_seed).reset_index(drop=True)
    ...
    for param_idx in range(len(final_params)):
        ...
        # Calculate one-tailed p_value, which is just taking all the values from the shuffled coef distribution greater than the final coeff and dividing by total number
        p_value = sum(shuffled_params[:, param_idx] > final_param) / len(shuffled_params[:, param_idx])
        print(f"P-value for {final_param_name}: {p_value:.4f}")
